...
const AllCompletedAppointments = () => {
  ...
  async function fetchServerData(search="", page=1, setloading=null) {
    ...
    let newQueryParams = { ...QueryParams("getAllCompletedAppointmentsByDoctor", "?") }
    ...
    const { data } = await getRequest(Routesfiles(newQueryParams)?.getAllCompletedAppointmentsByDoctor, setloading)
  ...
  return (
    <>
      <div>
        {
          <DataTable
            title="Old Completed Appointment Records  ( Patient )"
...
export default AllCompletedAppointments
