...
const DailyLabAppointment = () => {
  ...
  async function fetchServerData(setloading) {
    ...
    let newQueryParams = { ...QueryParams("getAllTodayLaptTestAppointmentByPathology", "?") }
    newQueryParams.setQuery += `locationId=${id}`

    const { data } = await getRequest(Routesfiles(newQueryParams)?.getAllTodayLaptTestAppointmentByPathology, setloading)
  ...
  return (
    <>
      <div>
        {
          <DataTable
            title="Daily Appointment Records  ( Patient )"
...
export default DailyLabAppointment
